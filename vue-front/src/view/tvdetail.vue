        ...
        >
          <div class="card d-flex flex-row">
            ...
            <div class="main-image text-center"> <!-- 가운데 정렬 추가 -->
              ...
              <img
                v-else
                :src="require(`../../../node-back/uploads/${item.tv_img}`)"
                alt="..."
                width="50%"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    async submitReview() {
      if (this.reviewText.trim() !== "" && this.selectedStars > 0) {
        ...
        try {
          const tv_no = this.$route.params.tv_no; // 현재 페이지의 tv_no 가져오기
          const response = await axios.post(
            `http://localhost:3000/trip/trip/${tv_no}/reviews`,
            newReview
          );
    ...
    async fetchFestival(tv_no) {
      try {
        console.log(`Fetching festival with tv_no: ${tv_no}`);
        const response = await axios.get(
          `http://localhost:3000/trip/trip/${tv_no}`
        );
        console.log("Festival response data:", response.data);
        this.contentList = response.data.map((item) => ({
          title: item.tv_tit,
          detail: item.tv_content,
          address: item.tv_ads,
          tv_img: item.tv_img,
          subImages: [
            item.tv_sb_img,
    ...
    async likeCheck() {
      ...
      const response = await axios.post(
        `http://localhost:3000/trip/likeCheck/${this.$route.params.tv_no}/${user_no}`
      );
    ...
    async likeInsert() {
      if (this.user.user_no === "") {
        ...
        await axios.post(
          `http://localhost:3000/trip/likeInsert/${this.$route.params.tv_no}/${this.user.user_no}`
        );
        this.isLiked = true;
        await this.getLike();
      }
    ...
    async likeDelete() {
      if (this.user.user_no === "") {
        ...
        await axios.post(
          `http://localhost:3000/trip/likeDelete/${this.$route.params.tv_no}/${this.user.user_no}`
        );
        this.isLiked = false;
        await this.getLike();
      }
    ...
    async getLike() {
      try {
        const tv_no = this.$route.params.tv_no;
        const response = await axios.get(
          `http://localhost:3000/trip/likeCount/${tv_no}`
        );
  ...
  mounted() {
    const tv_no = this.$route.params.tv_no; // 현재 페이지의 tv_no 가져오기
    console.log(`Mounted component with tv_no: ${tv_no}`);
    this.fetchFestival(tv_no);
    this.fetchReviews();
  },
};
</script>
